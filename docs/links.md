...
Always link to these URLs. All non-root URLs use a trailing slash to match the
site's canonical and sitemap.
...
| Purpose | URL |
| --- | --- |
| Sitemap index | https://www.romanticode.com/sitemap-index.xml |
...
- Marketplace publisher: `ruilegendoc`

---

## Rules for external use

- Always use `www.` subdomain. The apex `romanticode.com` 308-redirects to
  `www.romanticode.com`.
- Always use trailing slash for page URLs. Non-slash URLs 308-redirect to the
  slash version. Static assets (`robots.txt`, `sitemap-*.xml`, `favicon.svg`,
